...
/**
 * Comprehensive Amazon DynamoDB construct that provisions fully configured NoSQL tables with advanced features including secondary indexes,
 * encryption, backup, and auto-scaling.
 *
 * <p>
 * This construct handles the complete lifecycle of DynamoDB table creation with support for:
 *
 * <p>
 * <b>Core Features:</b>
 *
 * <ul>
 * <li><b>Table Configuration</b> - Partition/sort keys, attributes, billing modes
 * <li><b>Secondary Indexes</b> - Global Secondary Indexes (GSI) and Local Secondary Indexes (LSI)
 * <li><b>Encryption</b> - KMS-based encryption at rest with custom or managed keys
 * <li><b>Backup & Recovery</b> - Point-in-time recovery and backup vault integration
 * <li><b>Auto-scaling</b> - Dynamic capacity scaling based on utilization
 * <li><b>Streaming</b> - DynamoDB Streams for change data capture
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li>Dynamic attribute schema building with type inference
 * <li>Conditional index creation based on configuration
 * <li>Flexible billing mode support (On-Demand vs Provisioned)
 * <li>Custom table classes (Standard vs Infrequent Access)
 * <li>Template-based configuration with runtime value injection
 * <li>Comprehensive tagging strategy with inheritance
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li><b>Global Secondary Indexes</b> - Independent partition/sort keys with separate capacity
 * <li><b>Local Secondary Indexes</b> - Alternative sort key sharing partition key
 * <li><b>Projection Types</b> - Keys only, include specific attributes, or all attributes
 * <li><b>Auto-scaling</b> - Independent scaling policies per index
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li>KMS encryption with customer-managed or AWS-managed keys
 * <li>IAM-based access control integration
 * <li>VPC endpoint support for private access
 * <li>Backup encryption and retention policies
 * </ul>
 *
 * <p>
 * <b>Configuration Processing:</b> The construct supports complex configuration scenarios including conditional resource creation,
 * template-based parameter injection, and dynamic schema building based on application requirements.
 *
 * <p>
 * <b>Usage Example:</b>
 *
...
public class DynamoDbConstruct extends Construct {
  ...
  public DynamoDbConstruct(Construct scope, Common common, Table conf) {
    ...
    var table = Builder
      ...
      .tableName(conf.name())
      .partitionKey(Attribute.builder().name(conf.partitionKey().name()).type(conf.partitionKey().type()).build())
  ...
  private static GlobalSecondaryIndexPropsV2 globalSecondaryIndex(Index index) {
    var gsi = GlobalSecondaryIndexPropsV2.builder().indexName(index.name()).projectionType(index.projectionType());

    if (index.sortKey() != null) {
      gsi.sortKey(Attribute.builder().name(index.sortKey().name()).type(index.sortKey().type()).build());
    }
  ...
  private static LocalSecondaryIndexProps localSecondaryIndex(Index index) {
    var lsi = LocalSecondaryIndexProps.builder().indexName(index.name()).projectionType(index.projectionType());

    if (index.sortKey() != null) {
      lsi.sortKey(Attribute.builder().name(index.sortKey().name()).type(index.sortKey().type()).build());
    }
  ...
  private void billing(Table conf, Builder table) {
    var ok =
      BooleanUtils.oneHot(new boolean[] {conf.billing().onDemand(), conf.billing().fixed() != null, conf.billing().provisioned() != null});
  ...
  private void sortKey(Table conf, Builder table) {
    if (conf.sortKey() != null && conf.sortKey().name() != null && conf.sortKey().type() != null) {
      table.sortKey(Attribute.builder().name(conf.sortKey().name()).type(conf.sortKey().type()).build());
    }
  }
}
