 ...
 *
 * <ul>
 * <li><b>AWS Account</b> - Cross-account access via AccountPrincipal
 * <li><b>Federated</b> - SAML and OIDC federated identity integration
 * <li><b>ARN</b> - Direct ARN-based principal specification
 * <li><b>Service</b> - AWS service principals (lambda.amazonaws.com, etc.)
 * <li><b>Star (*)</b> - Public access principal for open policies
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li><b>Composite Principals</b> - Multiple principals combined in single policy
 * <li><b>Conditional Access</b> - IAM policy conditions for fine-grained control
 * <li><b>OIDC Integration</b> - EKS service account and pod identity trust relationships
 * <li><b>Session Tagging</b> - Automatic session tag support for federated access
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li><b>Service Account Binding</b> - Maps Kubernetes service accounts to IAM roles
 * <li><b>Pod Identity</b> - Direct pod-to-IAM role association
 * <li><b>Audience Validation</b> - STS audience verification for security
 * <li><b>Namespace Isolation</b> - Kubernetes namespace-based access control
 * </ul>
 *
 * <p>
 ...
 *
 * <ul>
 * <li>Condition-based access control for enhanced security
 * <li>OIDC issuer validation for EKS integration
 * <li>Proper audience restriction for token validation
 * <li>Namespace-based isolation for multi-tenant environments
 * </ul>
 *
 * <p>
 ...
 * Principal composite = Principal
 *   .builder()
 *   .type(PrincipalType.SERVICE)
 *   .value("lambda.amazonaws.com")
 *   .composite(List.of(ec2Principal, fargatePrincipal))
 *   .conditions(Map.of("StringEquals", conditionMap))
 *   .build();
 * }</pre>
 *
 * @param type
      ...
      .withConditions(
        Map
          .of(
            "StringEquals",
            CfnJson.Builder
              .create(scope, String.format("oidc-principal-%s", sa.role().name()))
              .value(
                Map
                  .of(
                    String.format("%s:aud", oidc.getOpenIdConnectProviderIssuer()),
                    "sts.amazonaws.com",
      ...
      .withConditions(
        Map
          .of(
            "StringEquals",
            CfnJson.Builder
              .create(scope, String.format("oidc-principal-%s", pod.role().name()))
              .value(
                Map
                  .of(
                    String.format("%s:aud", oidc.getOpenIdConnectProviderIssuer()),
